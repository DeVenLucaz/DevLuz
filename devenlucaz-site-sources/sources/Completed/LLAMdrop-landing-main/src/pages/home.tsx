...
const TIERS = [
  ...
  { name: "Mid",         badge: "T3", ram: "6–12 GB avail",  ramGb: 11.9, color: "text-primary",     borderColor: "border-primary/40",     bg: "bg-primary/5",    device: "Modern Linux laptops, Pi 5 8GB",       models: ["Gemma 3 12B", "Qwen3 8B", "Phi-4 14B", "Mistral NeMo 12B"],                      backend: "llama.cpp / Vulkan" },
  { name: "High",        badge: "T4", ram: "12–24 GB+ avail", ramGb: 9999, color: "text-yellow-400",  borderColor: "border-yellow-400/40",  bg: "bg-yellow-400/5", device: "High-end Linux machines, SBCs",        models: ["Gemma 3 27B", "Qwen3 32B", "DeepSeek R1 32B"],                                   backend: "llama.cpp / Vulkan" },
];
...
function detectBackend(specs: DeviceSpecs): BackendResult {
  ...
  if (os === 'macOS' || os === 'Windows') {
    return { backend: 'Unsupported OS', note: 'v0.10.0 hyper-focuses on Android, Linux, and SBCs' };
  }
  return { backend: 'Dynamic Prober (CPU vs Vulkan)', note: 'Runs a 10s benchmark on first boot to guarantee fastest backend' };
}
...
function CliDemo() {
  ...
  const SubHdr = () => (
    <div className="px-3 py-1.5 border-b border-border/20 bg-black/60 font-mono text-[10px] leading-[1.55] flex-shrink-0">
      ...
      <div className="text-muted-foreground/50">v0.10.0 - Free forever - GPL v3 - github.com/DeVenLucaz/llamdrop</div>
      ...
      case 'doctor': return (
        <div className="font-mono text-[9px] sm:text-[10px] space-y-1.5">
          {[
            ...
            { label: "Model catalog", items: ["✓ models.json valid — v0.10.0, verified models, updated 2026-06-19"] },
      ...
      <section className="pt-24 sm:pt-36 pb-14 sm:pb-24 px-4 sm:px-6 max-w-6xl mx-auto">
        ...
        >
          ...
          <p className="text-lg sm:text-xl md:text-2xl text-muted-foreground max-w-2xl mb-5 sm:mb-6 leading-relaxed">
            You shouldn't need to know what quantization is, which backend to install, or which model fits your RAM. llamdrop figures all of that out — silently, automatically, for any device from a phone to a Raspberry Pi.
          </p>

          <div className="flex flex-wrap gap-3 sm:gap-5 mb-10 sm:mb-14 text-xs sm:text-sm font-mono text-muted-foreground">
            {["5 device tiers", "Verified model catalog", "HuggingFace live search", "Dynamic backend prober", "No configuration"].map((s) => (
      ...
      <FadeIn className="py-14 sm:py-24 px-4 sm:px-6 bg-card border-y border-border">
        <div className="max-w-6xl mx-auto">
          <div className="mb-12">
            ...
            <h2 className="text-2xl sm:text-3xl md:text-5xl font-bold mb-4">Verified model catalog.<br />The entire HuggingFace GGUF library.</h2>
            <p className="text-lg text-muted-foreground max-w-2xl">
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            <div className="border border-primary/40 bg-primary/5 p-5 sm:p-8 relative overflow-hidden">
              ...
              <p className="text-muted-foreground mb-6 leading-relaxed">
                Models with confirmed performance data from users across platforms — Reddit, Discord, GitHub issues, and community testing. Every entry has validated RAM requirements and proven quantization settings for each tier.
              </p>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              ...
              { icon: Laptop,         label: "Chromebooks",                  note: "Via Linux development environment (Crostini). ARM64 or x86_64." },
              { icon: Cpu,            label: "Orange Pi / SBCs",             note: "Support for various single-board computers running ARM64 Linux." },
            ].map((item) => (
