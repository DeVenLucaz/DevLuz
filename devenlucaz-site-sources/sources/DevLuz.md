...
- Hindi (DAAYAN): Noto Serif Devanagari

**Signature element:** Astro-ink-drop as a persistent Three.js 3D character — modeled procedurally from the reference image at `sources/astro-inkdrop-reference.png`. Overlaid on the entire site in its own renderer. Wanders, breathes, reacts to cursor and scroll, occasionally interrupts the viewer. The most important single element on the site. See full behavior spec in the DosCom project section above.

**Motion:**
- Three.js / WebGL hero — go full 3D, earn it, don't fake it
- Astro-ink-drop — Three.js 3D, its own renderer, always present, always alive
- Scroll reveals: sharp and fast, like content being uncovered not floated in
...
- Never: "I'm a passionate developer who loves building amazing experiences"
- Never: skill percentage bars
- Never: "Let's work together!" CTA
- Never: fake testimonials
- Write like DeVen talks — direct, a little irreverent, confident without performing confidence
- The Termux constraint is stated plainly, not humbly: "Built entirely on an Android phone. No PC."
- FactRadar's instability is not hidden — "runs on free APIs, intentionally"
- DAAYAN gets literary copy. The only place on the site where the language changes register entirely.

---

## What Visitors Should Feel

1. **Impressed / slightly intimidated** — "this person does things I didn't think were possible on a phone"
2. **Curious** — "who actually is this person"
3. **Nothing is being sold to them** — the site just exists, it doesn't want anything from you

---

## Structure Note

Antigravity decides the structure. But: sections should feel like distinct worlds, not just scroll stops. DAAYAN gets its own dedicated page — not a section, a destination. The rest of the site can be whatever structure earns the content best.
